...
class ItemTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // The drag handle sits outside Dismissible's hit-test region: nesting it
    // inside meant a real finger's diagonal jitter at drag-start put the
    // handle's ImmediateMultiDragGestureRecognizer and Dismissible's
    // HorizontalDragGestureRecognizer in the same gesture arena, which read
    // as "glitchy" reordering (a flash of the delete background, stutter).
    return Container(
      decoration: BoxDecoration(
        color: item.packed ? const Color(0xFFF7FBF8) : null,
        border: const Border(bottom: BorderSide(color: Color(0xFFFAF3EC))),
      ),
      child: Row(
        children: [
          Expanded(
            child: Dismissible(
              ...
              onDismissed: (_) {
                ...
                messenger.showSnackBar(SnackBar(
                  ...
                  // SnackBar.persist defaults to true whenever an action is
                  // set, which disables the auto-dismiss timeout — without
                  // this the toast never goes away on its own.
                  persist: false,
                ));
              },
              child: InkWell(
                          ...
                          children: [
                            Text(
                              ...
                              style: TextStyle(
                                ...
                                color:
                                    item.packed ? AppColors.struckText : null,
                            ...
                            if (item.tag != null) ...[
                              ...
                              // Tap the pill to edit/remove the tag without
                              // hunting for the long-press edit sheet.
                              GestureDetector(
